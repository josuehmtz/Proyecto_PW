...
export class DataServices implements OnInit {
  ...
  cargarPersonas() {
    return this.httpClient.get(
      'https://proyectopw-4fc02-default-rtdb.firebaseio.com/personas.json'
    );
  }
  ...
  eliminarPersona(index: number) {
    let url: string;
    url =
      'https://proyectopw-4fc02-default-rtdb.firebaseio.com/personas ' +
      index +
      '.json';
    this.httpClient.delete(url).subscribe(
      (response) => console.log('resultado de eliminar usuario: ' + response),
  ...
  modificarPersona(index: number, persona: Persona) {
    ...
    this.httpClient.put(url, persona).subscribe(
      (response) => console.log('resultado de modificar usuario: ' + response),
